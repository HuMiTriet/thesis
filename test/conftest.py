...
def setup_maekawa_scale(
    ...
    for port in client_ports_number:
        process = Process(target=client_maekawa_app.run, kwargs={"port": port})
        client_processes.append(process)
        process.start()
...
def setup_token_ring_scale(
    ...
    for port in client_ports_number:
        process = Process(target=client_token_app.run, kwargs={"port": port})
        client_processes.append(process)
        process.start()
...
def result_aggregator():
    ...
    with open("scalling_token_ring.json", "w", encoding="utf-8") as f:
